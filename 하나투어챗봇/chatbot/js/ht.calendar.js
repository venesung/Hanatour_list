    ...
    var wddoObj = function (options) {
                ...
                if (datepickerTarget.length === 2 && datepickerTarget.is('input')) {
                    ...
                    if (startDate !== null) {
                        if (thisTime === startDate.getTime()) { //체크인
                            returnValue = (opts.plan) ? ' ui-checkin-state-active' : ' ui-checkin-state-active unplan'; //modify 3.1.0
                        } else {
                            if (stopDate !== null && opts.plan) { //modify 3.1.0
                        ...
                        //체크인과 아웃 같은 날짜 체크 시
                        if (stopDate !== null && thisTime === startDate.getTime() && thisTime === stopDate.getTime()) returnValue = ' ui-checkinout-state-active'; //add 3.1.0
                    }
                }
            }
        ...
        function settingInlineRange(data) {
            ...
            datepickerTarget.on('selectDay.wddo', function (e, data) {
                ...
                if (!opts.plan || startDate !== null && stopDate !== null) { //체크인, 체크아웃 존재
                    //체크인 설정
        ...
        function changeDate(_date, _target) {
            ...
            if (target.is('div') && opts.range) {  //인라인 range 형태 체크인 설정 
                ...
                triggerRange(); //인라인 전용 외부 호출 함수 //add 3.0.9
            } else {
        ...
        return {
            ...
            setPlan : function (value) { //미정 유무 변경 (inline 전용) //add 3.1.0
                ...
                selectDate(getPickerDate('checkin'));
            },
