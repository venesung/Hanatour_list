 ...
 *   startMaxDay:number                                  //체크아웃 달력 체크인 보다 몇일 뒤부터 선택할지 [default 1]
    ...
    var wddoObj = function (options) {
        var scope,
            ...
            init = function (target, options) {
                ...
                if (target.length > 0) {
                    ...
                    initCallback(); //updateTitle(), settingInlineRange(), settingRange() 내부 처리
                    
                    initLayout();
                    initEvent();
                    
                    if (datepickerTarget.is('input') && input.length > 1) { //인풋에 체크인&아웃 인 경우.. add 2.2.1
                        if (opts.rangeSelect !== undefined && opts.rangeSelect.length > 0) { //박수있은 경우
                            initNight(); //박수 //opts.autoValue 는 함수 내부 처리
                        }
                    }

                    if (datepickerTarget.is('div') && !opts.range) changeDate(new Date()); // 일반 인라인 달력
                }
            };
        ...
        function getDefaultOption() {
            return {
                ...
                beforeShowDay: function (date) {
                    ...
                    return [showDate(date), holiday + weekendday(date) + range];
                },
                /* datepicker 없는 옵션*/
                ...
                if (datepickerTarget.length === 2 && datepickerTarget.is('input')) {
                    ...
                    if (startDate !== null) {
                        if (thisTime === startDate.getTime()) { //체크인
                            ...
                            if (stopDate !== null) {
                                if (thisTime > startDate.getTime() && thisTime < stopDate.getTime()) { //체크인 아웃 사이
                                    returnValue = ' ui-datepicker-range';
                                } else if (thisTime === stopDate.getTime()) { //체크아웃
                                    returnValue = ' ui-checkout-state-active';
                                }
                            }
                        }
                        ...
                        //체크인과 아웃 같은 날짜 체크 시
                        //if (stopDate !== null && thisTime === startDate.getTime() && thisTime === stopDate.getTime()) returnValue = ' ui-checkinout-state-active';
                    }
                }
            }
        ...
        function initCallback() {
            ...
            datepickerTarget.on('update.wddo', function (e, data) {
                ...
                if (datepickerTarget.is('div') && opts.range) triggerRange(); //인라인 전용 외부 호출 함수 //add 3.0.4
        ...
        function updateTitle(data) {
            ...
            //멀티시 오른쪽 화살표 위치 조정(멀티중 첫번째만 화살표가 있기때문에 )
        }
        ...
        function settingInlineRange(data) {
            ...
            datepickerTarget.on('selectDay.wddo', function (e, data) {
                ...
                if (startDate !== null && stopDate !== null) { //체크인, 체크아웃 존재
                    //체크인 설정
        ...
        //체크인 & 아웃 date 외부로 던져줄 opts.onRange 발생 시키는 함수
        function triggerRange() {
            var startDate = $.datepicker.formatDate(opts.dateFormat, getPickerDate('checkin'), opts);
            var stopDate = $.datepicker.formatDate(opts.dateFormat, getPickerDate('checkout'), opts);

            //인라인 전용 외부 호출 함수
            if (opts.onRange !== undefined) opts.onRange([startDate, stopDate]);
        }
        ...
        function settingRange(data) {
            ...
            getPicker('checkout').datepicker('option', 'beforeShow', function (input, inst) {
                ...
                //30일 제한
                var thirtyDate = new Date(checkinDate);
        ...
        function getMinMaxDate(kind, targetDate) {
            ...
            if (kind === 'min') {
                ...
                //minDate 보다 작으면
                returnDate = (targetDate.getTime() - limitDate.getTime() < 0) ? limitDate : targetDate;
            } else if (kind === 'max') {
        ...
        function changeRange(value) {
            ...
            var isDate = (value instanceof Date && !isNaN(value.valueOf())); //date 형태 인지 유무 //add 3.0.4
            var stopDate; //체크아웃 date

            //체크아웃 Date 객체 정의 
            if (isDate) { //date 형태이면
                stopDate = getMinMaxDate('max', value); //maxDate 한계에 넘지 않는 선에서 Date 선택 add 3.0.4
            } else {
                stopDate = getTargetDate(startDate, value); //value에 맞는 Date 객체 설정 //modify 3.0.4
            }
        ...
        function changeDate(_date, _target) {
            ...
            //add 3.0.4
            if (target.is('div') && opts.range) {  //인라인 range 형태 체크인 설정 
        ...
        //inline range 형태에 선택한 날짜에 맞게 스크롤 이동 함수
        function scrollToSelectMonth(_target) {
            var target = _target || datepickerTarget;

            if (target.length > 0 && target.is('div') && opts.range) {  //인라인 range 형태
                var ypos = target.find('.ui-state-active').closest('.ui-datepicker-group').offset().top;
                var distance = target.position().top;

                $(document).scrollTop(ypos - distance);
            }
        }
